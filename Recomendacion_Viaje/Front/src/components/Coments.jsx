...
const CommentSection = ({ packageId }) => {
  ...
  const [valuePackage, setvaluePackage] = useState(packageId.comments);
  const { idCard } = useParams();
  const handleRatingClick = (index) => {
    setRating(index + 1);
  };
  const { fetchData } = usePostPackage(idCard, setvaluePackage, valuePackage);
  useEffect(() => {
    setvaluePackage(packageId?.package?.comments);
  }, [packageId]);
  ...
  return (
    <div className="p-4 rounded-lg shadow-lg w-1/2 ml-60 -mt-28 overflow-hidden ">
      ...
      <div>
        {valuePackage?.length === 0 ? (
          <p>No hay comentarios aún.</p>
        ) : (
          valuePackage?.map((item, index) => (
            <div
              key={index}
              className="mb-4 p-2 border border-gray-200 rounded-md shadow-sm  "
            >
              <div className="flex pb- ">
                ...
                <span className="flex  w-32 h-6 rounded-lg shadow-lg justify-center mr-2 ml-6 ">
                  <div className="flex items-center p-2 ">
                    ...
                    <h1 className="font-bold ml-1 text-sm text-center content-center">{`${item?.name} ${item?.lastName}`}</h1>
                  </div>
                </span>
              </div>
